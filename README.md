...
The `debug?` variable in [`config.cljs`](src/cljs/cljs_fake_news/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns aarandela.cljs-fake-news.example
  (:require [aarandela.cljs-fake-news.config :as config])
